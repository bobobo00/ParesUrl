...
/*public class parseUrl {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("主机："+url.substring(url.indexOf("//")+2,url.indexOf("/s")));
        System.out.println("端口：");
        if(url.indexOf('#')!=-1){
            System.out.println("请求信息："+url.substring(url.indexOf('?')+1,url.indexOf('#')));
            message=url.substring(url.indexOf('?')+1,url.indexOf('#'));

            System.out.println("文档片段标识符："+url.substring(url.indexOf('#')+1,url.length()));
        }else{
            System.out.println("请求信息："+url.substring(url.indexOf('?')+1,url.length()));
             message=url.substring(url.indexOf('?')+1,url.length());
            System.out.println("文档片段标识符：");
        }
